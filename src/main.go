...
func clientLimiterKey(r *http.Request) string {
	...
	if mac, err := getMacAddress(ip); err == nil {
		if normalized := merchant.NormalizeMACAddress(mac); normalized != "" && normalized != sentinelMACAddress {
...
// a customer for a device that does not exist. It is never an identity — and it
// must never key a quota, or two unresolvable clients would share one bucket.
...
type lightningInvoiceResponse struct {
	...
	// Code is a stable machine-readable refusal reason, additive to the
	// `status`/`error` pair the shipped portal already reads: values so far are
	// `device-unresolved` (errDeviceUnresolvedCode), `quote-rate-limited`,
	// `request-too-large` and `amount-too-large`.
	// RetryAfter mirrors the Retry-After header in the body so a portal can
	// render a countdown without reaching for a header it may not be allowed to
	// read.
