...
PKG_VERSION:=0.0.$(shell git rev-list --count HEAD ^main)+$(shell git rev-parse --short HEAD)
PKG_RELEASE:=1
PKG_FLAGS:=overwrite
...
define Build/Compile
	...
	GOARCH=$(GOARCH) \
	GOMIPS=$(GOMIPS) \
	go build -o $(PKG_NAME) -trimpath -ldflags="-s -w" github.com/OpenTollGate/tollgate-module-basic-go/src
endef
