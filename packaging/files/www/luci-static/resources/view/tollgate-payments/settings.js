	...
	render: function(data) {
			...
			var section = E('div', { 'class': 'cbi-section' }, [
				...
				E('div', { 'class': 'cbi-page-actions tg-actions-sm' }, [
					E('button', { 'class': 'cbi-button', 'click': function() {
						...
						if (!tbody) return;
						var n = tbody.children.length;
						var newShare = 1.0 / (n + 1);
						var squeeze = n / (n + 1);
						for (var i = 0; i < n; i++) {
							var cur = parseFloat(q('cfg_ps_' + i + '_factor').value) || 0;
							writeRow(i, cur * squeeze);
						}
						var idx = n;
						tbody.appendChild(E('tr', {}, [
							E('td', { 'class': 'tg-slider-td' }, [
								E('input', {
									'type': 'range', 'id': 'cfg_ps_' + idx + '_range',
									'min': '0', 'max': '100', 'step': '1', 'value': String(Math.round(newShare * 100)),
									'class': 'tg-slider',
									'data-idx': String(idx),
								}),
								E('input', {
									'type': 'hidden', 'id': 'cfg_ps_' + idx + '_factor',
									'value': newShare.toFixed(4)
								})
							]),
							E('td', { 'class': 'tg-td tg-pct-label' }, [
								E('span', { 'id': 'cfg_ps_' + idx + '_pct' }, (newShare * 100).toFixed(1) + '%')
							]),
							E('td', { 'class': 'tg-td' }, [
		...
		function walletDrain() {
			ui.showModal(_('Drain Wallet'), [
				E('p', _('This will convert ALL wallet funds (testnut mints only) to Cashu tokens and save them to a file on the device.')),
				E('div', { 'class': 'right' }, [
					E('button', { 'class': 'cbi-button', 'click': ui.hideModal }, _('Cancel')), ' ',
					E('button', { 'class': 'cbi-button cbi-button-remove', 'click': function() {
						ui.hideModal();
						...
						cliJson('wallet', 'drain', 'cashu').then(function(resp) {
							...
							if (d.saved_to) lines.push('Saved to: ' + d.saved_to, '');
							...
							stateSpan('wl_drain_state', d.saved_to ? 'Drained → ' + d.saved_to : 'Drained.', 'tg-state-success');
							cliJson('wallet', 'balance').then(function(r) {
								var el = q('ov_balance');
								if (el && r && r.data) el.textContent = (r.data.balance_sats || 0) + ' sats';
							});
