...
from conftest import UPSTREAM_URL, client_mac, ecash_wallet, upstream_health
...
class TestReadOnlyModes:
    def test_list_offers_parses_real_advertisement(self, upstream_health):
        r = run_clientd("--gateway", "upstream", "--mac", client_mac,
                        "--list-offers")
        assert r.returncode == 0, r.stderr
        assert "metric=milliseconds" in r.stdout
        assert "1 sat/step" in r.stdout
        assert "http://mint:8085" in r.stdout

    def test_status_json_no_session(self, upstream_health, clean_session):
        r = run_clientd("--gateway", "upstream", "--mac", client_mac,
                        "--status", "--json")
        assert r.returncode == 0, r.stderr
    ...
    def test_waybar_json_shape(self, upstream_health, clean_session):
        r = run_clientd("--gateway", "upstream", "--mac", client_mac,
                        "--waybar")
        assert r.returncode == 0, r.stderr
...
def test_dry_run_never_pays(upstream_health, clean_session):
...
def test_daemon_pays_and_renews(upstream_health, ecash_wallet, clean_session):
