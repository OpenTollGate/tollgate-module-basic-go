...
- **DLEQ proofs stripped before wallet.Receive() to handle mint keyset
  rotation.** gonuts-tollgate v0.6.1 verifies DLEQ proofs against the
  mint's current active keyset, but `proof.Id` identifies the keyset
  that actually signed the proof. When a mint rotates keysets, tokens
  minted under the old keyset fail with "invalid DLEQ proof". The mint
  validates proof signatures server-side during the swap operation, so
  skipping client-side DLEQ verification does not allow spending
  invalid tokens.
